...
initSum:= Module[
   ...
   propDen[p_,m_,w_]:=((m^2-SC[p,p])^2+(m*w)^2)/(m^2-SC[p,p]) /. substitutions //Simplify;
   propDenSq[p_,m_,w_]:=((m^2-SC[p,p])^2+(m*w)^2) /. substitutions //Simplify;

   repl = {pow -> Power, sw2 -> 1 - pow[MW, 2]*pow[MZ, -2], cw -> pow[1 - sw2, 1/2], 
   sw -> pow[sw2, 1/2], vev -> 2*MW*sw*pow[EE, -1], pi -> Pi};
];
...
addToSum:=Module[
   ...
   intFunL= Simplify[Together[intFun- intFun0]];

   intFun0=Simplify[(intFun0 /. t->tMax)-(intFun0 /. t->tMin)];

   sum=sum + totFactor* Simplify[intFun0 + Simplify[(intFunL /. Log -> LLog)]];
